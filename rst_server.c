                  ...
                  u_char *packetptr)
{
    ...
    if (iphdr->ip_p == IPPROTO_TCP )
    {
        ...
        memset(rstack_packet, 0, sizeof(rstack_packet));
        ...
        ipHdr = (struct iphdr *) rstack_packet;
        tcpHdr = (struct tcphdr *) (rstack_packet + sizeof(struct iphdr));

        // Preparing RST-ACK packet
        ipHdr->ihl = 5;
        ipHdr->version = 4;
        ipHdr->tos = 0;
        ...
        ipHdr->check = csum((unsigned short *) rstack_packet, ipHdr->tot_len); 
        ...
        memcpy(synack_packet, rstack_packet, sizeof(struct iphdr) + sizeof(struct tcphdr));
        ...
        if (tcphdr->syn && !tcphdr->ack )
        {
            ...
            ipHdr = (struct iphdr *) rstack_packet;            
            if((bytes = sendto(sock, rstack_packet, ipHdr->tot_len, 0, (struct sockaddr *) &addr_in, sizeof(addr_in))) < 0)
                perror("Error on sendto()");
            else {
                printf("[+] [%s:%d] Sending RST-ACK\n", srcip, ntohs(tcphdr->source));
            }
            ipHdr = (struct iphdr *) synack_packet;            
